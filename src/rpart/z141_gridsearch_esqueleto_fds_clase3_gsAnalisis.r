...
archivo_salida <- "./exp/HT2020/gridsearch_clase3_BasesianAnalisis.txt"
...
for (vcp in c(-0.990)) { # Bucle más externo ahora para vcp
  for (vmax_depth in c(17)) {
    for (vmin_split in c(2002)) {
      for (vmin_bucket in c(998)) { # Itera sobre los valores para minbucket
